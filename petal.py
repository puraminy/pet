...
def filter_words(tokens: List[str], word_counts=None, max_words: int = -1):
    ...
    tokens = (
        ...
        for word in tokens
        if word[0] == "Ġ"
        and len([char for char in word[1:] if char.isalpha()]) >= 2
    )
    if word_counts and max_words > 0:
        tokens = sorted(
            tokens, key=lambda word: word_counts[word[1:]], reverse=True
        )[:max_words]
    return tokens
...
def get_word_to_id_map(
    ...
    word2id = {
        word[1:]: tokenizer.convert_tokens_to_ids(word) for word in words
    }
    logger.info(
...
def main():
    ...
    with open(
        os.path.join(args.output_dir, "config.txt"), "w", encoding="utf8"
    ) as fh:
